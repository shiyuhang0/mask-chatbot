...
function IconOpenAI({className, name,...props}: React.ComponentProps<'svg'>) {
  let avatarId = generateRandomString(10)
  let svgCode = multiavatar(name || avatarId)
  const svgElement = React.useMemo(() => {
    return {__html: svgCode}; // Wrap the svgCode in an object to use dangerouslySetInnerHTML
  }, [svgCode]);

  return (
      <div className={cn(
          'flex h-8 w-8 shrink-1 select-none items-center justify-center shadow'
